...
class simulation_model():
    def __init__(self):
        ...
        'infectiousPopulationI'          : lambda t: ( (2.0) if ( t  <=  self.starttime ) else (self.memoize('infectiousPopulationI',t-self.dt) + self.dt * ( self.memoize('infectionRateIr',t-self.dt) - ( self.memoize('recoveryRateRr',t-self.dt) ) )) ),
        ...
        'susceptiblePopulationS'          : lambda t: ( (18.0) if ( t  <=  self.starttime ) else (self.memoize('susceptiblePopulationS',t-self.dt) + self.dt * ( -1 * ( self.memoize('infectionRateIr',t-self.dt) ) )) ),
        
    
        # Flows
        'infectionRateIr'             : lambda t: max([0 , ( self.memoize('contactRateC', t) * self.memoize('susceptiblePopulationS', t) * self.memoize('infectivityI', t) ) * ( self.memoize('infectiousPopulationI', t) / self.memoize('totalPopulationN', t) )]),
        'recoveryRateRr'             : lambda t: max([0 , self.memoize('infectiousPopulationI', t) / self.memoize('averageDurationD', t)]),
        
    
        # converters
        'averageDurationD'      : lambda t: 10.0,
        'contactRateC'      : lambda t: 2.0,
        'infectivityI'      : lambda t: 0.2,
        'totalPopulationN'      : lambda t: 20.0,
        ...
        self.stocks = ['infectiousPopulationI',   'recoveredPopulationR',   'susceptiblePopulationS'  ]
        self.flows = ['infectionRateIr',   'recoveryRateRr'  ]
        self.converters = ['averageDurationD',   'contactRateC',   'infectivityI',   'totalPopulationN'  ]
        self.gf = []
        self.constants= []
        self.events = [
            ]
